...
test.describe('Verify Cinema Data Accuracy: UI vs API', () => {
    ...
    test('Branch Tab displays correct data', async ({ homePage }) => {
        ...
        let uiCinemas: Locator[];

        await test.step(`Get all UI-displayed cinemas to iterate`, async () => {
            uiCinemas = await homePage.cinemaShowtimesTabs.cinemaTabs.all();
        });
    ...
    test('Movie-Showtime Tab displays correct data', async ({ homePage }) => {
        ...
        await test.step(`Loop through all UI-displayed cinemas and branches and verify showtimes data`, async () => {
